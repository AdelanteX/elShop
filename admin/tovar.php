...
else
{
  ...
  //создадим переменную экшен в нее методом ГЕТ поместим значение из урла
  //Сделаем проверку если в экшене есть ключевое слово УДАЛИТЬ то делаем запрос в БД и удаляем товар.  
    $action = $_GET["action"];
if (isset($action))
{
   $id = (int)$_GET["id"]; 
   ...
   if ($_SESSION['delete_tovar'] == '1')
   {
           ...
           $delete = mysqli_query($connection, "DELETE FROM table_products WHERE products_id = '$id'");  
...
<?php
    ...
    //Показать количество товаров из БД
    $all_count = mysqli_query($connection, "SELECT * FROM table_products");
...
<?php
 ...
 $result1 = mysqli_query($connection, "SELECT * FROM category WHERE type='mobile'");
...
<?php
 $result1 = mysqli_query($connection, "SELECT * FROM category WHERE type='notebook'");
  If (mysqli_num_rows($result1) > 0)
{
$row1 = mysqli_fetch_array($result1);
do
...
<?php
 $result1 = mysqli_query($connection, "SELECT * FROM category WHERE type='notepad'");
  If (mysqli_num_rows($result1) > 0)
{
$row1 = mysqli_fetch_array($result1);
do
...
$count = mysqli_query($connection, "SELECT COUNT(*) FROM table_products $cat");
...
if ($temp[0] > 0)   
{
$result = mysqli_query($connection, "SELECT * FROM table_products $cat ORDER BY products_id DESC LIMIT $start, $num",$link);
